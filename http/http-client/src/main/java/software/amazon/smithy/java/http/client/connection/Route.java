 ...
 *
 * <p>Connections to the same route can be pooled and reused.
 * Two routes are equal if they connect to the same destination
 * via the same path (including proxy configuration).
 *
 * <p>This is the key type used in {@link ConnectionPool} to identify
 * which connections are interchangeable.
 *
 * <p><b>Important:</b> Routes are compared by value, not identity.
 * Two Route instances with the same scheme, host, port, and proxy
 * configuration are considered equal and will share connections.
 *
 * <p><b>Example:</b>
 * <pre>{@code
 ...
 *
 * // These are the same route - connections can be shared
 * assert route1.equals(route2);
 *
 * // Different route - different connection pool
