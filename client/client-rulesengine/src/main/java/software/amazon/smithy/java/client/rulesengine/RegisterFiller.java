...
abstract class RegisterFiller {
    ...
    protected RegisterFiller(
        ...
        if (registerTemplate.length != registerDefinitions.length) {
            throw new IllegalArgumentException(String.format(
                    ...
                    registerTemplate.length, registerDefinitions.length));
        }
    ...
    static RegisterFiller of(Bytecode bytecode, Map<String, Function<Context, Object>> builtinProviders) {
        ...
        if (registerDefinitions.length - 1 < 64) {
            return new FastRegisterFiller(registerDefinitions, builtinIndices, hardRequiredIndices,
                                          inputRegisterMap, builtinProviders, registerTemplate);
        } else {
            return new LargeRegisterFiller(registerDefinitions, builtinIndices, hardRequiredIndices,
                                           inputRegisterMap, builtinProviders, registerTemplate);
        }
    }
