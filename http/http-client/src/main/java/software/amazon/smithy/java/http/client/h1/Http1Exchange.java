 ...
 *   <li>Response is read via {@link #statusCode()}, {@link #responseHeaders()}, {@link #responseBody()}</li>
 * </ol>
 *
...
public final class Http1Exchange implements HttpExchange {
     ...
     */
    Http1Exchange(Http1Connection connection, HttpRequest request, Route route, byte[] lineBuffer) throws IOException {
     ...
     *
     * <p>Waits up to 1 second for 100 Continue response:
     * <ul>
    ...
    private int readLine(UnsyncBufferedInputStream in) throws IOException {
        return in.readLine(responseLineBuffer, Http1Connection.RESPONSE_LINE_BUFFER_SIZE);
    }
    ...
    private static boolean containsChunked(String value) {
        ...
        if (value.indexOf(',') >= 0) {
            for (String token : value.split(",")) {
                ...
                if (HttpHeaders.normalizeHeaderName(token).equals("chunked")) {
                    return true;
                }
            }
        }
