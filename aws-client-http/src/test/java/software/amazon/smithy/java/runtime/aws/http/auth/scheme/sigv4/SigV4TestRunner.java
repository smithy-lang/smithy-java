...
public class SigV4TestRunner {
    ...
    private static final class TestCase {
        ...
        public final SmithyHttpRequest request;
        public final SmithyHttpRequest expected;

        private TestCase(String name, Context context, SmithyHttpRequest request, SmithyHttpRequest expected) {
            this.name = name;
        ...
        public static TestCase from(Path directory) {
            ...
            var context = Context.load(directory, name);
            var request = loadRequest(directory, name);
            var signed = loadSigned(directory, name);
        ...
        private static SmithyHttpRequest loadRequest(Path directory, String name) {
            String fileName = directory.resolve(REQUEST).toString();
            return parseRequest(fileName);
        }

        private static SmithyHttpRequest loadSigned(Path directory, String name) {
    ...
    record Context(
        ...
        static Context load(Path directory, String name) {
        ...
        private static AuthProperties getAuthProperties(ObjectNode objectNode) {
            return AuthProperties.builder()
                .put(SigningProperties.SERVICE, objectNode.expectStringMember("service").getValue())
                .put(SigningProperties.REGION, objectNode.expectStringMember("region").getValue())
                .put(
                    SigningProperties.TIMESTAMP,
