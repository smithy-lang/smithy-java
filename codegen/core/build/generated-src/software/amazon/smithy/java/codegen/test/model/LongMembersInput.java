...
public final class LongMembersInput implements SerializableStruct {
    ...
    static final Schema SCHEMA = Schema.structureBuilder(ID)
        .putMember("requiredLongs", PreludeSchemas.LONG,
            new RequiredTrait())
        .putMember("optionalLongs", PreludeSchemas.LONG)
        .putMember("defaultShort", PreludeSchemas.LONG,
            new DefaultTrait(Node.from(1)))
        .build();

    private static final Schema SCHEMA_REQUIRED_LONGS = SCHEMA.member("requiredLongs");
    private static final Schema SCHEMA_OPTIONAL_LONGS = SCHEMA.member("optionalLongs");
