...
@SmithyGenerated
public final class DoublesInput implements SerializableShape {
    ...
    static final class InnerSerializer implements BiConsumer<DoublesInput, ShapeSerializer> {
        static final InnerSerializer INSTANCE = new InnerSerializer();

        @Override
        public void accept(DoublesInput shape, ShapeSerializer serializer) {
            serializer.writeDouble(SCHEMA_REQUIRED_DOUBLE, shape.requiredDouble);
            if (shape.optionalDouble != null) {
                serializer.writeDouble(SCHEMA_OPTIONAL_DOUBLE, shape.optionalDouble);
            }
            serializer.writeDouble(SCHEMA_DEFAULT_DOUBLE, shape.defaultDouble);
        }
    }
