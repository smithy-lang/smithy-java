...
@SmithyGenerated
public final class LongsInput implements SerializableShape {
    ...
    static final class InnerSerializer implements BiConsumer<LongsInput, ShapeSerializer> {
        static final InnerSerializer INSTANCE = new InnerSerializer();

        @Override
        public void accept(LongsInput shape, ShapeSerializer serializer) {
            serializer.writeLong(SCHEMA_REQUIRED_LONGS, shape.requiredLongs);
            if (shape.optionalLongs != null) {
                serializer.writeLong(SCHEMA_OPTIONAL_LONGS, shape.optionalLongs);
            }
            serializer.writeLong(SCHEMA_DEFAULT_SHORT, shape.defaultShort);
        }
    }
