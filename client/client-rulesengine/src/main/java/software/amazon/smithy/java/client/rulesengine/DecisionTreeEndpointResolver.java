...
final class DecisionTreeEndpointResolver implements EndpointResolver {
    ...
    DecisionTreeEndpointResolver(EndpointRuleSet rules, List<RulesExtension> extensions) {
        this.rules = rules;
    ...
    public CompletableFuture<Endpoint> resolveEndpoint(EndpointResolverParams params) {
        try {
            ...
            var endpointParams = ContextProvider.createEndpointParams(
                    operationContextParams, params.context(), operation, params.inputValue());
    ...
    private Endpoint convertEndpoint(EndpointResolverParams params, EndpointValue ev) {
        ...
        builder.uri(EndpointUtils.parseUri(ev.getUrl()));
