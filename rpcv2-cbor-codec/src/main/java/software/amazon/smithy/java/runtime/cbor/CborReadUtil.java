...
public final class CborReadUtil {
    private static final int DITTO_HASH_PRIME = 92821;
    ...
    // The hash functions in this class must be identical to Ditto's Constant::hash. Any differences will cause
    // fields to be unresolvable.
    public static int hashString(byte[] buf, int off, int len) {
        if (CborParser.isIndefinite(len)) {
            return hashStringIndefinite(buf, off, CborParser.itemLength(len));
        } else {
            return hashStringFinite(buf, off, len);
        }
    }

    private static int hashStringIndefinite(byte[] buf, int off, int len) {
        int hash = 0;
        int hashed = 0;
        while (hashed < len) {
            byte b = buf[off];
            int minor = b & MINOR_TYPE_MASK;
            int argLen = argLength(minor);
            int strLen = readStrLen(buf, off, minor, argLen);
            off += argLen + 1;
            for (int i = off; i < off + strLen; i++) {
                hash += buf[i];
                hash *= DITTO_HASH_PRIME;
            }
            off += strLen;
            hashed += strLen;
        }
        if (hashed != len) throw new BadCborException("cannot hash unclosed indefinite length string");
        return hash & Integer.MAX_VALUE;
    }

    private static int hashStringFinite(byte[] buf, int off, int len) {
        int lim = off + len;
        int hash = 0;
