 ...
 *
 * <p>Manages a single TCP socket for HTTP/1.1 communication. HTTP/1.1 allows
 * only one request/response exchange at a time (no multiplexing like HTTP/2).
 *
 * <h2>Connection Reuse</h2>
 * <p>Supports HTTP/1.1 persistent connections (keep-alive). After each exchange,
 * the connection can be returned to the pool for reuse if:
 * <ul>
 ...
 *
 * <h2>Thread Safety</h2>
 * <p>This class is thread-safe for {@link #newExchange(HttpRequest)} - only one
 * exchange can be active at a time. Concurrent calls to {@code newExchange()}
 * will fail with an exception if another exchange is already active.
 *
 * <h2>Proxy Support</h2>
 * <p>If created through an HTTP proxy with CONNECT tunnel (for HTTPS), the
 * underlying socket is already connected through the tunnel. All proxy handshaking
 * happens during connection establishment, not in this class.
 */
public final class Http1Connection implements HttpConnection {
    ...
    private static final InternalLogger LOGGER = InternalLogger.getLogger(Http1Connection.class);
     ...
     */
    public Http1Connection(Socket socket, Route route, Duration readTimeout) throws IOException {
        this.socket = socket;
    ...
    public HttpExchange newExchange(HttpRequest request) throws IOException {
        ...
        try {
            return new Http1Exchange(this, request, route, lineBuffer);
        } catch (IOException e) {
     ...
     *
     * <p>Called by {@link Http1Exchange} when the exchange completes.
     */
     ...
     *
     * <p>Called by {@link Http1Exchange} after parsing response headers.
     ...
     *
     * <p>Called by {@link Http1Exchange} when errors occur during I/O.
     */
    void markInactive() {
     ...
     * @return the same socket, now tunneled to target
     * @throws IOException if CONNECT tunnel establishment fails
     */
    public static Socket establishConnectTunnel(
            Socket proxySocket,
            String targetHost,
            int targetPort,
        ...
        try {
            ...
            if (statusCode == 200) {
                ...
                throw new IOException(
                        "Proxy authentication required (407). Check proxy credentials.");
                ...
                throw new IOException(
                        "Unexpected proxy response: " + statusLine);
            }
