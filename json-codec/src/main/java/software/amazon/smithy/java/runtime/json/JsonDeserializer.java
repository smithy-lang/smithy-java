...
final class JsonDeserializer implements ShapeDeserializer {
    ...
    JsonDeserializer(byte[] source, boolean useJsonName, TimestampFormatter defaultTimestampFormat,
    ...
    private SdkSchema resolveMember(SdkSchema schema, String field) {
        ...
        if (useJsonName) {
            ...
            member = schema.findMember(m -> m.getTrait(JsonNameTrait.class).map(JsonNameTrait::getValue)
                    .filter(n -> n.equals(field)).isPresent());
        }
