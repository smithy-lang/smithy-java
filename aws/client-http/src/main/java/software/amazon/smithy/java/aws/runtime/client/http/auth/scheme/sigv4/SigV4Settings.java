...
import software.amazon.smithy.java.aws.runtime.client.http.auth.ClockSetting;
import software.amazon.smithy.java.aws.runtime.client.http.auth.RegionSetting;
 ...
 * }</pre>
 */
public interface SigV4Settings extends ClockSetting, RegionSetting {
    /**
    ...
    @Configuration(description = "Set the signing name to use for SigV4 signing.")
    default void signingName(Context context, String signingName) {
        if (signingName == null || signingName.isEmpty()) {
            throw new IllegalArgumentException("signingName cannot be null or empty");
        }
        context.put(SIGNING_NAME, signingName);
    }
}
