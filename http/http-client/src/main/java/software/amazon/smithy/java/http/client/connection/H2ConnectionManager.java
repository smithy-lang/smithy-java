...
final class H2ConnectionManager {
    ...
    H2Connection acquire(Route route) throws IOException {
        ...
        // Slow path: create or pick under limit
        synchronized (state) {
            ...
            H2Connection[] next = new H2Connection[snapshot.length + 1];
            System.arraycopy(snapshot, 0, next, 0, snapshot.length);
            next[snapshot.length] = newConn;
            state.conns = next;
    ...
    private H2Connection tryAcquire(H2Connection[] conns) {
        ...
        for (H2Connection conn : conns) {
            if (conn == null || !conn.canAcceptMoreStreams() || !conn.validateForReuse()) {
                continue;
            }

            int active = conn.getActiveStreamCount();
