...
final class CborParser {
    // note: we can only support up to 32 token types due to the 5-bit size of `context` values in Ditto
    // we currently have 21 token types (excluding FINISHED)
    public static final class Token {
        ...
        // simple types are all represented in ditto program as (token, offset, len)
        ...
        // FINISHED is not recorded in the Ditto program, so it can be smaller than our 5-byte context
    ...
    private byte endOfBuffer(int i) {
        ...
        overhead = 0;
        if (i > len) {
            throw new BadCborException("over-read");
        }
        if (inCollection) {
            throwIncompleteCollectionException();
        }
