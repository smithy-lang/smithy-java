...
-Dfory.enable_unsafe_memory_access=false
```

The same module opening lets Smithy's optional compact-string writer inspect
immutable Java string backing bytes. It is an accelerator, not a runtime
requirement: without the opening, Smithy uses the supported `String.charAt`
writer and produces identical output.
...
Independent short forks reproduced the optimized M/L paths:

- M deserialization: 1,967-2,011 ns/op and 4,264 B/op;
- L deserialization: 19,399-19,471 ns/op and 36,152 B/op;
- opened compact-string M serialization: 1,744 and 1,774 ns/op;
- no-opening M serialization fallback: 1,989 ns/op.
...
Smithy deserialization is now statistically level with Fory on M/L and
...
- packed canonical schema-field probes;
- schema-selected scalar and timestamp readers;
- whole-number double and ten-digit epoch-second fast tiers;
- generated raw enum matching with exact-byte collision checks;
- primitive required-member masks in generated builders;
- schema-order framing fusion;
- self-contained nested readers that let C2 scalar-replace builders;
- compact ASCII string construction and map-key reads;
- opportunistic compact-string ASCII serialization.

The final M CPU profile contains neither generic epoch `parseLong` nor
`StandardUnit.from`. Nested `MetricDatum.Builder` and `Dimension.Builder`
objects are absent from the allocation profile.

Rejected experiments include atomic reader pooling, uncached scalar strings,
medium-string caching, staged maps, zero-capacity maps, unsafe loads, internal
zero-copy string construction, one-pass VarHandle string stores, fixed
8-24-byte string stores, writer thread-local retention, replacing pool stores,
and a monomorphic publication cache. Each regressed latency or failed the 2%
stability contract.

Results are in:

- `build/perf-study/runtime-codegen-optimization/fory-comparison-optimized.json`;
- `build/perf-study/runtime-codegen-optimization/compact-string-writer-a1.json`;
- `build/perf-study/runtime-codegen-optimization/compact-string-writer-opened-m.json`;
- `build/perf-study/runtime-codegen-optimization/compact-string-writer-fallback-m.json`;
- `build/perf-study/runtime-codegen-optimization/async/raw-enum-final-2/`;
- `build/perf-study/runtime-codegen-optimization/async/compact-string-writer-m-cpu/`;
- `build/perf-study/runtime-codegen-optimization/perfasm/`.
...
cold sample. The optimized CloudWatch root is 26,495 bytes; raw enum matching
adds about 2.5 KiB for the 27-value `StandardUnit` schema. A cold sample took
about 46 ms. These are individual roots, not a distribution. The shaded
`codec-commons` artifact decreased from 423,614 to 315,371 bytes after ASM
...
Continue the generated JSON backend as an internal experiment. Keep it disabled
by default and do not enable the AWS JSON integration flag in production.
Codec-level CloudWatch performance now justifies continued work, but it does
not satisfy the deferred protocol, multi-fork, lifecycle, or resource gates.
