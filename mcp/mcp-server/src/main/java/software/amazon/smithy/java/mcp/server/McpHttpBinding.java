...
final class McpHttpBinding {
    ...
    static String protocolVersionFromMetadata(JsonRpcRequest request) {
        var params = request.getParams();
        var metadata = isObject(params) ? params.getMember("_meta") : null;
        return isObject(metadata) ? stringMember(metadata, McpWireNames.PROTOCOL_VERSION) : null;
    }
    ...
    static boolean isObject(Document document) {
        return document != null
                && (document.isType(ShapeType.MAP) || document.isType(ShapeType.STRUCTURE));
    }
    ...
    static int statusCode(
        ...
        if (response.getError().getCode() == -32700 || response.getError().getCode() == -32600) {
            return 400;
        }
        if (!statelessClaim) {
            return 200;
