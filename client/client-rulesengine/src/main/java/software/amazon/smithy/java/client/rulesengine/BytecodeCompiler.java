...
final class BytecodeCompiler {
    ...
    private void compileEndpointRule(EndpointRule rule) {
        ...
        if (!e.getHeaders().isEmpty()) {
            for (var entry : e.getHeaders().entrySet()) {
                ...
                writer.writeByte(Opcodes.CREATE_LIST);
                writer.writeByte(entry.getValue().size());
                // Now the header name
                writer.writeByte(Opcodes.LOAD_CONST);
                writer.writeByte(writer.getConstantIndex(entry.getKey()));
            }
            // Combine the N headers that are on the stack
            writer.writeByte(Opcodes.CREATE_MAP);
            writer.writeByte(e.getHeaders().size());
        }
        ...
        if (!e.getProperties().isEmpty()) {
            ...
            writer.writeByte(Opcodes.CREATE_MAP);
            writer.writeByte(e.getProperties().size());
        }
    ...
    private void compileExpression(Expression expression) {
        expression.accept(new ExpressionVisitor<Void>() {
            ...
            public Void visitGetAttr(GetAttr getAttr) {
                ...
                int attrIndex = writer.getConstantIndex(AttrExpression.from(getAttr));
                if (attrIndex < 256) {
                    writer.writeByte(Opcodes.GET_ATTR);
                    writer.writeShort(attrIndex);
                } else {
                    throw new RulesEvaluationError("Too many attr expressions");
                }
                return null;
            }
            ...
            public Void visitBoolEquals(Expression left, Expression right) {
                if (left instanceof BooleanLiteral b) {
                    ...
                    writer.writeByte(getFunctionIndex("booleanEquals"));
                }
                return null;
            }
    ...
    private void compileLiteral(Literal literal) {
        if (literal instanceof StringLiteral s) {
            ...
            if (st.expressionCount() == 0) {
                addLoadConst(st.resolve(0, null));
                ...
                // String templates need to push their template placeholders
                st.forEachExpression(this::compileExpression);
                int templateIndex = writer.getConstantIndex(st);
                writer.writeByte(Opcodes.RESOLVE_TEMPLATE);
                writer.writeShort(templateIndex);
            }
            ...
            writer.writeByte(Opcodes.CREATE_LIST);
            writer.writeByte(t.members().size());
            ...
            writer.writeByte(Opcodes.CREATE_MAP);
            writer.writeByte(r.members().size());
    ...
    private Bytecode buildProgram() {
        var registerDefs = new RegisterDefinition[registerAllocator.getRegistry().size()];
        registerAllocator.getRegistry().toArray(registerDefs);

        var fns = new RulesFunction[usedFunctions.size()];
        usedFunctions.toArray(fns);

        return writer.build(registerDefs, fns, bdd.getNodes(), bdd.getRootRef());
    }
}
