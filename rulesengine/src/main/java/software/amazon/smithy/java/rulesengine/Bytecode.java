...
public final class Bytecode {
    ...
    static final byte COND_GENERAL = 0;
    ...
    static final byte COND_SUBSTRING_EQ = 6;
        ...
        for (int i = 0; i < conditionOffsets.length; i++) {
            ...
            if (offset + 2 < len) {
                ...
                boolean isReturn = next == (Opcodes.RETURN_VALUE & 0xFF)
                        || next == (Opcodes.SET_REG_RETURN & 0xFF);
                ...
                if (isReturn) {
                    switch (firstOpcode) {
                        ...
                        default -> {
                            /* fall through to wider pattern checks */ }
                    }
                }
            }
            ...
            if (offset + 4 < len && (bytecode[offset] & 0xFF) == (Opcodes.STRING_EQUALS_REG_CONST & 0xFF)) {
                ...
                if (next == (Opcodes.RETURN_VALUE & 0xFF) || next == (Opcodes.SET_REG_RETURN & 0xFF)) {
                    conditionTypes[i] = COND_STRING_EQ_REG_CONST;
                    conditionOperands[i] = (constIdx << 8) | reg;
                }
            }

            // COND_GENERAL for everything else
        }
    }
