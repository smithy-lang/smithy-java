...
public final class BlobMembersInput implements SerializableStruct {
    ...
    static final Schema SCHEMA = Schema.structureBuilder(ID)
        .putMember("requiredBlob", PreludeSchemas.BLOB,
            new RequiredTrait())
        .putMember("optionalBlob", PreludeSchemas.BLOB)
        .putMember("streamingBlob", SharedSchemas.STREAMING_BLOB,
            new RequiredTrait())
        .build();

    private static final Schema SCHEMA_REQUIRED_BLOB = SCHEMA.member("requiredBlob");
    private static final Schema SCHEMA_OPTIONAL_BLOB = SCHEMA.member("optionalBlob");
