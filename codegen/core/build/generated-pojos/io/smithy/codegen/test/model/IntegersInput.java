...
@SmithyGenerated
public final class IntegersInput implements SerializableShape {
    ...
    static final class InnerSerializer implements BiConsumer<IntegersInput, ShapeSerializer> {
        static final InnerSerializer INSTANCE = new InnerSerializer();

        @Override
        public void accept(IntegersInput shape, ShapeSerializer serializer) {
            serializer.writeInteger(SCHEMA_REQUIRED_INT, shape.requiredInt);
            if (shape.optionalInt != null) {
                serializer.writeInteger(SCHEMA_OPTIONAL_INT, shape.optionalInt);
            }
            serializer.writeInteger(SCHEMA_DEFAULT_INT, shape.defaultInt);
        }
    }
