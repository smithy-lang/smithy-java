...
public final class StdioMcpClient extends McpRemoteClient {
    ...
    public static class Builder {
         ...
         * forever. Defaults to 5 minutes, symmetric with {@link HttpMcpClient}.
         */
    ...
    protected JsonRpcResponse exchange(JsonRpcRequest request) {
        ...
        try {
            return pending.await(requestTimeout);
        } finally {
    ...
    private static final class PendingResponse {
        ...
        JsonRpcResponse await(Duration timeout) {
            try {
                var completed = result.poll(timeout.toMillis(), MILLISECONDS);
                if (completed == null) {
                    throw new McpRemoteException("Timed out waiting for MCP response");
                }
