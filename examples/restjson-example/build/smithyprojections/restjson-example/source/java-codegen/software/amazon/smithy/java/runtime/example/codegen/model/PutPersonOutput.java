...
public final class PutPersonOutput implements SerializableShape {
    ...
    private static final SdkSchema SCHEMA_STRUCT = SdkSchema.memberBuilder(7, "struct", Nested.SCHEMA)
        .id(ID)
        .build();
    ...
    @Override
    public int hashCode() {
        return Objects.hash(name, favoriteColor, age, birthday, status, list, set, struct);
    }
            ...
            decoder.readStruct(SCHEMA, (member, de) -> {
                ...
                switch (index) {
                    ...
                    case 5 -> {
                                  ...
                                  de.readList(member, elem -> result.add(de.readString(elementSchema)));
                                  list(result);
                              }
                    case 6 -> {
                                  ...
                                  de.readList(member, elem -> {
                                      if (result.add(de.readString(elementSchema))) {
                                          throw new SdkSerdeException("Duplicate item in unique list " + elem);
                                      }
                                  });
                                  set(result);
                              }
                    case 7 -> struct(Nested.builder().deserialize(de).build());
                }
            });
            return this;
        }
