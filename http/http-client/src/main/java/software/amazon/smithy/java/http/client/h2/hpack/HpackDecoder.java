...
public final class HpackDecoder {
            ...
            if ((b & 0x80) != 0) {
                ...
                if (headerFieldSeen) {
                    throw new IOException(
                            "Dynamic table size update MUST occur at beginning of header block (RFC 7541 Section 4.2)");
                }
    ...
    private void validateHeaderName(String name) throws IOException {
        for (int i = 0; i < name.length(); i++) {
            ...
            if (c >= 'A' && c <= 'Z') {
                throw new IOException(
                        "Header field name contains uppercase character: '" + name + "' (RFC 9113 Section 8.2)");
            }
        }
    }
}
