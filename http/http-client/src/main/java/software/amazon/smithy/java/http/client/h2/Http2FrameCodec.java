...
import static software.amazon.smithy.java.http.client.h2.Http2Constants.ERROR_FRAME_SIZE_ERROR;
...
import static software.amazon.smithy.java.http.client.h2.Http2Constants.FLAG_PADDED;
import static software.amazon.smithy.java.http.client.h2.Http2Constants.FLAG_PRIORITY;
import static software.amazon.smithy.java.http.client.h2.Http2Constants.FRAME_HEADER_SIZE;
import static software.amazon.smithy.java.http.client.h2.Http2Constants.FRAME_TYPE_CONTINUATION;
...
import static software.amazon.smithy.java.http.client.h2.Http2Constants.FRAME_TYPE_PRIORITY;
...
import static software.amazon.smithy.java.http.client.h2.Http2Constants.frameTypeName;
...
final class Http2FrameCodec {
    ...
    Http2FrameCodec(InputStream in, OutputStream out, int maxFrameSize) {
        this.in = in;
        this.out = out;
    ...
    Frame readFrame() throws IOException {
        ...
        if (length > maxFrameSize) {
            throw new Http2Exception(ERROR_FRAME_SIZE_ERROR, "Frame size " + length + " exceeds " + maxFrameSize);
        }
    ...
    byte[] readHeaderBlock(Frame initialFrame) throws IOException {
        ...
        if (initialFrame.type() == FRAME_TYPE_PUSH_PROMISE && initialPayload != null) {
            if (initialLength < 4) {
                throw new Http2Exception(ERROR_FRAME_SIZE_ERROR,
    ...
    private void validateFrameSize(int type, int flags, int length) throws Http2Exception {
        switch (type) {
     ...
     * Validate stream ID requirements per RFC 9113.
     */
    private void validateStreamId(int type, int streamId) throws Http2Exception {
        switch (type) {
     ...
     */
    private byte[] removePadding(byte[] payload, int frameType) throws Http2Exception {
        if (payload.length < 1) {
            throw new Http2Exception(ERROR_FRAME_SIZE_ERROR,
     ...
     */
    private byte[] removePriority(byte[] payload) throws Http2Exception {
     ...
     * to the output stream (e.g., via Http2Connection's writer thread).
     *
     * @param type frame type
     * @param flags frame flags
     * @param streamId stream identifier
    ...
    record Frame(int type, int flags, int streamId, byte[] payload, int length) {
         ...
         * @throws Http2Exception if frame is invalid
         */
        int[] parseSettings() throws Http2Exception {
         ...
         * @throws Http2Exception if frame is invalid
         */
        int[] parseGoaway() throws Http2Exception {
            if (type != FRAME_TYPE_GOAWAY) {
                throw new Http2Exception(ERROR_PROTOCOL_ERROR, "Expected GOAWAY frame, got " + frameTypeName(type));
            } else if (payload == null || length < 8) {
                throw new Http2Exception(ERROR_FRAME_SIZE_ERROR, "GOAWAY frame payload too short: " + length);
            }
         ...
         * @throws Http2Exception if frame is invalid or increment is zero
         */
        int parseWindowUpdate() throws Http2Exception {
            ...
            if (increment == 0) {
                throw new Http2Exception(ERROR_PROTOCOL_ERROR, "WINDOW_UPDATE increment must be non-zero");
            }
         ...
         * @throws Http2Exception if frame is invalid
         */
        int parseRstStream() throws Http2Exception {
