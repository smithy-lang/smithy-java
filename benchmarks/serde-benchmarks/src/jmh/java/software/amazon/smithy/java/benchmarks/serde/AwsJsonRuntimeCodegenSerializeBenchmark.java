...
/**
 * A/B benchmark for production-shaped runtime codegen over AWS JSON PutItem requests.
 */
@State(Scope.Benchmark)
public class AwsJsonRuntimeCodegenSerializeBenchmark {
    ...
    @Param({"awsJson1_0_PutItemRequest_MixedItem_M"})
    ...
    @Benchmark
    public ByteBuffer codecSerialize() {
        return codec.serialize(state.input);
    }

    @Benchmark
    public void protocolSerialize(Blackhole blackhole) {
        blackhole.consume(protocol.createRequest(operation(), state.input, state.context, state.endpoint));
    }
