...
@SmithyGenerated
public final class BytesInput implements SerializableShape {
    ...
    static final class InnerSerializer implements BiConsumer<BytesInput, ShapeSerializer> {
        static final InnerSerializer INSTANCE = new InnerSerializer();

        @Override
        public void accept(BytesInput shape, ShapeSerializer serializer) {
            serializer.writeByte(SCHEMA_REQUIRED_BYTE, shape.requiredByte);
            if (shape.optionalByte != null) {
                serializer.writeByte(SCHEMA_OPTIONAL_BYTE, shape.optionalByte);
            }
            serializer.writeByte(SCHEMA_DEFAULT_BYTE, shape.defaultByte);
        }
    }
