...
final class StreamImplNumber {
    ...
    static void writeInt(final JsonStream stream, int value) throws IOException {
        stream.ensure(12);
        byte[] buf = stream.buf;
    ...
    static void writeLong(final JsonStream stream, long value) throws IOException {
        stream.ensure(22);
        byte[] buf = stream.buf;
        int pos = stream.count;
        if (value < 0) {
    ...
    static void writeFloat(JsonStream stream, float val) throws IOException {
        if (Float.isNaN(val)) {
            stream.writeVal("NaN");
            return;
        }
    ...
    static void writeDouble(JsonStream stream, double val) throws IOException {
        if (Double.isNaN(val)) {
            stream.writeVal("NaN");
            return;
        }
