...
public final class PutPersonOutput implements SerializableShape {
    ...
    private PutPersonOutput(Builder builder) {
        ...
        this.set = builder.set != null ? Collections.unmodifiableSet(builder.set) : null;
    ...
    private static final class InnerSerializer implements BiConsumer<PutPersonOutput, ShapeSerializer> {
        private static final InnerSerializer INSTANCE = new InnerSerializer();

        @Override
        public void accept(PutPersonOutput shape, ShapeSerializer serializer) {
            ...
            if (shape.status != null) {
                serializer.writeInteger(SCHEMA_STATUS, shape.status);
            }
            if (shape.list != null) {
                serializer.writeList(SCHEMA_LIST, shape.list, SharedSchemas.ListOfStringSerializer.INSTANCE);
            }
            if (shape.set != null) {
                serializer.writeList(SCHEMA_SET, shape.set, SharedSchemas.SetOfStringSerializer.INSTANCE);
            }
            serializer.writeList(SCHEMA_REQUIRED_LIST, shape.requiredList, SharedSchemas.ListOfStringSerializer.INSTANCE);
            if (shape.struct != null) {
                shape.struct.serialize(serializer);
            }
        }
    }
        ...
        private static final class InnerDeserializer implements ShapeDeserializer.StructMemberConsumer<Builder> {
            ...
            public void accept(Builder builder, SdkSchema member, ShapeDeserializer de) {
                switch (member.memberIndex()) {
                    ...
                    case 5 -> builder.list(SharedSchemas.deserializeListOfString(member, de));
                    case 6 -> builder.set(SharedSchemas.deserializeSetOfString(member, de));
                    case 7 -> builder.requiredList(SharedSchemas.deserializeListOfString(member, de));
                    case 8 -> builder.struct(Nested.builder().deserialize(de).build());
                }
            }
        }
    }
