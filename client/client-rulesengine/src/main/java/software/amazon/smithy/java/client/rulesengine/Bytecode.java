 ...
 * 6     GET_ATTR_EXPRESSION [length:2][UTF-8 bytes]
 * 7     STRING_TEMPLATE [length:2][UTF-8 bytes]
 * </pre>
 *
...
public final class Bytecode {
    ...
    static final byte CONST_GET_ATTR_EXPRESSION = 6;
    static final byte CONST_STRING_TEMPLATE = 7;
    ...
    Bytecode(byte[] bytecode, int[] conditionOffsets, int[] resultOffsets,
        ...
        this.conditionEndOffsets = computeConditionEndOffsets(conditionOffsets, resultOffsets, bytecode.length);
    }
    ...
    private static int[] computeConditionEndOffsets(int[] conditionOffsets, int[] resultOffsets, int bytecodeLength) {
        int[] endOffsets = new int[conditionOffsets.length];

        for (int i = 0; i < conditionOffsets.length; i++) {
            if (i + 1 < conditionOffsets.length) {
                // Next condition starts where this one ends
                endOffsets[i] = conditionOffsets[i + 1];
            } else if (resultOffsets.length > 0) {
                // Last condition ends where first result starts
                endOffsets[i] = resultOffsets[0];
            } else {
                // No results, condition goes to end of bytecode
                endOffsets[i] = bytecodeLength;
            }
        }
    ...
    /**
     * Gets the end offset for a condition (exclusive).
     *
     * @param conditionIndex the condition index
     * @return the end offset in the bytecode (exclusive)
     */
    public int getConditionEndOffset(int conditionIndex) {
        return conditionEndOffsets[conditionIndex];
    }
    ...
    /**
     * Create a registers array ready for evaluation of the bytecode.
     *
     * @param context Calling context.
     * @param parameters Input parameters.
     * @param builtinProviders Builtin providers.
     * @return the creates registers array.
     */
    public Object[] createRegisters(
            Context context,
            Map<String, Object> parameters,
            Map<String, Function<Context, Object>> builtinProviders
    ) {
        Object[] registers = registerTemplate.clone();

        for (Map.Entry<String, Object> entry : parameters.entrySet()) {
            Integer index = inputRegisterMap.get(entry.getKey());
            if (index != null) {
                registers[index] = entry.getValue();
            }
        }

        // Only fill builtins that don't have defaults
        for (int i : builtinIndices) {
            if (registers[i] == null) {
                Function<Context, Object> provider = builtinProviders.get(registerDefinitions[i].builtin());
                if (provider != null) {
                    registers[i] = provider.apply(context);
                }
            }
        }

        for (int i : hardRequiredIndices) {
            if (registers[i] == null) {
