...
public final class PutPersonOutput implements SerializableShape {
    ...
    private static final SdkSchema SCHEMA_NESTED = SdkSchema.memberBuilder("nested", Nested.SCHEMA)
        .id(ID)
        .build();
    ...
    @Override
    public int hashCode() {
        return Objects.hash(name, favoriteColor, age, birthday, status, list, set, requiredList, nested);
    }
    ...
    public void serialize(ShapeSerializer serializer) {
        serializer.writeStruct(SCHEMA, st -> {
            ...
            if (set != null) {
                st.writeList(SCHEMA_SET, stl -> {
                    for(var setElem : set) {
                        stl.writeString(SCHEMA_SET_MEMBER, setElem);
                    }
                });
            };
            st.writeList(SCHEMA_REQUIRED_LIST, stl -> {
                for(var requiredListElem : requiredList) {
